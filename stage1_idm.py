...
def train_step():
    ...
    return wm_loss, sigreg_loss, loss
...
def test_step():
    ...
    wm_loss = wm.label(batch, return_emb=False)
...
for step in loop(cfg.stage1.steps + 1, desc="[green bold](stage-1) Training IDM + FDM"):
    wm_loss_train, vq_loss, loss = train_step()

    if step % 500 == 0:
        wm_loss_test = test_step()


    if step > 0 and (step % 1000 == 0 or step == cfg.stage1.steps):
        torch.save(
            dict(
